...
**rehydrate 熔断（同源缺陷修复）**：gap 触发的 `session/hydrate` 重试必须受控——快照重放（replay）窗口内的同步递归重试延迟到宏任务执行；连续无进展（snapshot cursor 未推进）的 hydrate 按 250ms 起指数退避、30s 封顶，cursor 推进即复位。禁止紧循环重试（实证：单个重复 frame 曾驱动 60s 内 6 626 次 `session/hydrate` RPC）。
