...
## 变更提案
- 提案名称：fix-startup-profile-llm-hot-reload
- 提案目录：logos/changes/fix-startup-profile-llm-hot-reload/
...
- Delta 文件：`logos/changes/fix-startup-profile-llm-hot-reload/deltas/prd/2-product-design/1-feature-specs/core-04-serve-api-design.md`
...
8. 所有变更合并完成后，自动执行 git commit（告知用户，无需确认）：
   git add -A && git commit -m "docs(fix-startup-profile-llm-hot-reload): merge spec deltas"
   然后提示用户：按更新后的规格实现代码，代码完成后运行 `openlogos verify` 验收，验收通过后明确授权执行 `openlogos archive fix-startup-profile-llm-hot-reload`。
