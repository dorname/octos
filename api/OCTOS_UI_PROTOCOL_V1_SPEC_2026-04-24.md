...
- `server/shutdown` (stops the serving process through the same graceful path
  as SIGINT; advertised and callable only on a local `--solo` HTTP serve,
  never to session-scoped connections, otherwise typed
